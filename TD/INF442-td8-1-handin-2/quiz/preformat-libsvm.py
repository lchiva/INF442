...
if __name__ == "__main__":
    if len(sys.argv) >= 3:
        ...
        print(
            "Syntax: python %s <file_to_read> <file_to_write> [<id_label> <rest_of_cols> ['<char_sep>'] ]" % sys.argv[0])
    ...
    with open(readfrom, "r") as f:
        for line in f:
            ...
            if label > len(cols):
                print(
                    "ERROR, indicated column for label does not exist (%d columns detected in file)" % len(cols))
            the_label = cols[label - 1]
            if the_label in ['0', '1']:  # switch 0 to -1
                the_label = '-1' * (the_label == '0') + \
                    '1' * (the_label == '1')
            cad = '%s ' % (the_label)
            index = 1
            ind_w = 1
