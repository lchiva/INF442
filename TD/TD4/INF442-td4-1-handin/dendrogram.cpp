...
{
    // TODO: Exercise 3.5
    int representative_1 = find(e->get_p1());
    int representative_2 = find(e->get_p2());
    if (representative_1 == representative_2)
    {
        return;
    }
    double h = e->get_length() * 0.5;
    if (rank[representative_1] > rank[representative_2])
    {
        parent[representative_2] = representative_1;
        left[representative_2] = down[representative_1];
        down[representative_1] = representative_2;
        height[representative_2] = h;
    }
    else if (rank[representative_1] < rank[representative_2])
    {
        parent[representative_1] = representative_2;
        left[representative_1] = down[representative_2];
        down[representative_2] = representative_1;
        height[representative_1] = h;
    }
    else
    {
        if (representative_1 < representative_2)
...
double dendrogram::get_cluster_height(int cluster)
{
    ...
    for (int i = 0; i < num_nodes; i++)
    {
        if (clusters[i] == cluster && i != cluster)
        {
            double len = height[i];
            if (len > h)
            {
