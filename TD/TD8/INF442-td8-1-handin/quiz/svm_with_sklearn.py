...
if __name__ == "__main__":
    if len(sys.argv) >= 3:
        ...
        print(
            "Syntax: python %s <dataset_train> <dataset_test> [<id_label> <rest_of_cols> ['<char_sep>'] ]" % sys.argv[0])
    ...
    if ".svm" in readfrom_train:
        ...
        # by default it's a sparse CSR matrix which does not work with MinMaxScaler
        ...
        dataset_train = pd.read_csv(readfrom_train,
                                    sep=char_sep,
                                    index_col=0,
                                    # skip comments describing scooter dataset
                                    skiprows=5 * \
                                    ("scooter-train" in readfrom_train),
                                    header=None)  # no name of columns
        dataset_test = pd.read_csv(readfrom_test, sep=char_sep, index_col=0,
                                   skiprows=5 *
                                   ("scooter-train" in readfrom_test),
                                   header=None)
    ...
    # to match libSVM's svm-train default parameters, change parameters here for Q6 and Q8
    svm_model = sk.svm.SVC(C=1, gamma='auto')
    # to match libSVM's svm-scale's default parameter
